...
from data.oxford_load import Multi_dataloader, DB_dataloader, ELE_dataloader
...
if __name__ == '__main__':
    ...
    model = EmbedNet(Bn_stream, Net_vlad).cuda()
    ...
    for epoch in range(200):
        ...
        for iteration in range(train_num):
            ...
            valid_data, ele_query, ele_pos, ele_neg = ELE_dataloader(query, training_queries, Pos_num,
                                                                                        Neg_num)
            if valid_data == 0:
                continue
        
            N_p, C, H, W = ele_pos.shape
            N_n, _, _, _ = ele_neg.shape

            ele_query = torch.unsqueeze(ele_query, 0)

            pos = ele_pos.view(B*N_p, C, H, W)
            neg = ele_neg.view(B*N_n, C, H, W)
            traindata = torch.cat([ele_query, pos, neg], 0).cuda()
